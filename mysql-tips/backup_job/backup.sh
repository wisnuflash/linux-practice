...
# === Daftar Job Backup ===
# Format: "NAMA_JOB|HOST|PORT|USER|PASSWORD"
JOBS=(
  "dblive|localhost|3306|root|password|new-dblive"
#  "panelcloud|192.168.23.25|3306|root|hfVyE75U9NcReAws|panelcloud"
)
...
backup_database() {
  ...
  for DB in $DBS; do
    ...
    mysqldump -u "$USER" -p"$PASSWORD" -h"$HOST" -P"$PORT" "$DB" > "$DUMP_FILE"
